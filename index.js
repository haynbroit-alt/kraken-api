...
const r=https.request({hostname:'api.elevenlabs.io',path:'/v1/text-to-speech/21m00Tcm4TlvDq8ikWAM',method:'POST',headers:{'xi-api-key':ELEVEN,'Content-Type':'application/json','Content-Length':d.length}},(res)=>{
...
if(audioPath){
try{execSync(ff+' -y -loop 1 -f lavfi -i color=c=0x1a1a2e:size=1280x720:rate=25 -i '+audioPath+' -c:v libx264 -c:a aac -shortest -pix_fmt yuv420p /tmp/v.mp4',{timeout:120000});return '/tmp/v.mp4';}catch(e){}
}
...
if(p==='/auth'){res.writeHead(302,{Location:'https://accounts.google.com/o/oauth2/v2/auth?client_id='+CLIENT_ID+'&redirect_uri='+encodeURIComponent(REDIRECT_URI)+'&response_type=code&scope='+encodeURIComponent('https://www.googleapis.com/auth/youtube.upload')+'&access_type=offline'});res.end();}
else if(p==='/callback'){const code=url.parse(req.url,true).query.code,d=JSON.stringify({code,client_id:CLIENT_ID,client_secret:CLIENT_SECRET,redirect_uri:REDIRECT_URI,grant_type:'authorization_code'});token=await post('oauth2.googleapis.com','/token',{'Content-Type':'application/json','Content-Length':d.length},d);res.writeHead(200);res.end('Connecte! <a href="/publish">Publier</a>');}
else if(p==='/publish'){
if(!token){res.writeHead(200);res.end('<a href="/auth">Se connecter</a>');return;}
