...
async function refreshToken(){
  ...
  const newToken=await post('oauth2.googleapis.com','/token',{'Content-Type':'application/json','Content-Length':Buffer.byteLength(d)},d);
...
async function audio(text){
  ...
  return new Promise((ok,ko)=>{
    const r=https.request({
      ...
      method:'POST',
      headers:{'xi-api-key':ELEVEN,'Content-Type':'application/json','Content-Length':Buffer.byteLength(d),'Accept':'audio/mpeg'}
    },(res)=>{
      const c=[];
      res.on('data',x=>c.push(x));
      res.on('end',()=>{
        fs.writeFileSync('/tmp/a.mp3',Buffer.concat(c));
        ok('/tmp/a.mp3');
      });
    });
    r.on('error',ko);
...
async function video(audioPath){
  ...
  if(audioPath&&fs.existsSync(audioPath)){
    try{
      ...
      console.log('audio video err:'+e.message);
    }
  }
...
async function upload(title,vpath){
  ...
  const meta=JSON.stringify({
    snippet:{title,description:title+' - Decouvrez cette histoire fascinante!',categoryId:'22',tags:['insolite','mystere','histoire']},
  ...
  return post('www.googleapis.com','/upload/youtube/v3/videos?part=snippet,status&uploadType=multipart',{
...
const topics=[
  ...
  'Un secret cache dans l histoire de France',
  'Une decouverte scientifique qui change tout',
  'Un phenomene naturel inexplique'
];

function getScript(topic){
  return 'Aujourd hui nous allons parler de: '+topic+
    '. C est une decouverte fascinante qui va vous surprendre. '+
    topic+' est l un des sujets les plus mysterieux de notre epoque. '+
    'Les scientifiques eux-memes n arrivent pas a l expliquer completement. '+
    'Des millions de personnes dans le monde entier se posent des questions sur ce sujet. '+
    'Restez avec nous jusqu a la fin pour decouvrir tous les secrets de '+topic+'.';
}
...
const server=http.createServer(async(req,res)=>{
  ...
  if(p==='/auth'){
    ...
    res.end('<h1>Connecte!</h1><p><a href="/publish">Publier une video maintenant</a></p>');
  }
    ...
    if(!token){
      if(fs.existsSync('/tmp/token.json')){
        ...
        res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'});
        res.end('<a href="/auth">Se connecter a YouTube</a>');
        return;
      }
    }
    res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'});
    ...
    try{
      ...
      res.end('<p>YouTube ID: '+(r.id||JSON.stringify(r).substring(0,300))+'</p>');
    }catch(e){
    ...
    res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'});
    res.end('<h1>Auto-publication activee!</h1><p>Une video sera publiee toutes les 24h.</p>');
    setInterval(async()=>{
      try{
        ...
        const vp=await video(ap);
        await upload(topic,vp);
        console.log('Video publiee automatiquement: '+topic);
      }catch(e){console.log('Erreur auto: '+e.message);}
    },24*60*60*1000);
  }

  else{
    res.writeHead(200,{'Content-Type':'text/html;charset=utf-8'});
    res.end('<h1>KRAKEN YouTube Bot</h1><ul><li><a href="/auth">1. Connecter YouTube</a></li><li><a href="/publish">2. Publier une video</a></li><li><a href="/auto">3. Activer auto 24h</a></li></ul>');
  }
});
